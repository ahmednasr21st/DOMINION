...
def _find_tool_script(tool_dir_name: str, script_name: str) -> str:
    """Find a tool script in ~/tools/ or /opt/ fallback."""
    import os
    home = Path.home()
    candidates = [
        home / "tools" / tool_dir_name / script_name,
        Path("/opt") / tool_dir_name / script_name,
        Path("/opt") / tool_dir_name.lower() / script_name,
    ]
    for c in candidates:
        if c.exists():
            return str(c)
...
def run_linkfinder(js_files: List[str], phase_dir: Path) -> List[str]:
    ...
    results = []
    script = _find_tool_script("linkfinder", "linkfinder.py")
    if not script:
        return results
    for js_url in js_files[:50]:
        rc, stdout, _ = run(
            f"python3 {script} -i {js_url} -o cli",
...
def run_secretfinder(js_files: List[str], phase_dir: Path) -> List[Dict]:
    ...
    secrets = []
    script = _find_tool_script("secretfinder", "SecretFinder.py")
    if not script:
        return secrets
